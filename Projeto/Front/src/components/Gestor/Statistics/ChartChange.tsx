...
//         return "#f80101"; // Cinza
//       case "Solo Exposto":
//         return "#0055ff"; // Marrom
//       default:
//         return "#2f7803dd"; // Cor padrão
//     }
//   };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const chartData: ChartData[] = Object.values(groupedData).map(data => {
          const total = data.classes.reduce((sum, item) => sum + item.value, 0);
          const percentageData = data.classes.map(item => ({
            ...item,
            value: (item.value / total) * 100,
          }));
          return {
            cidade: data.cidade,
            classes: percentageData,
          };
        });
